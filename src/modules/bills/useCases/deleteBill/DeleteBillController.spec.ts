...
describe('DeleteBillController', () => {
  ...
  it('should be able to delete an bill', async () => {
    const { token } = (
    ...
    const response = await request(app)
      .delete(`/bills/${bill.body.id}`)
      .set('Authorization', `Bearer ${token}`);

    expect(response.status).toBe(204);
  });

  it('should not be able to delete an bill without authenticate', async () => {
    const { token } = (
      await request(app).post('/auth').send({
        email: 'admin@admin.com',
        password: 'admin123',
    ...
    const response = await request(app).delete(`/bills/${bill.body.id}`);

    expect(response.status).toBe(401);
  });

  it('should not be able to delete an bill if bill does not exists', async () => {
    const { token } = (
      await request(app).post('/auth').send({
        email: 'admin@admin.com',
        password: 'admin123',
